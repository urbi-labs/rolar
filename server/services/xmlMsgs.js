...
const log = (text) => console.log(chalk.cyan.bgBlack("[redis]", text));
...
const { FTPI, FTLI, SFTLI } = require("../XMLMessages/GS1_Patterns.json");
const { LFTPI, FTLPN } = require("../XMLMessages/GS1_Patterns.json");
const { UUID, DISP } = require("../XMLMessages/GS1_Patterns.json");

const company_prefix = config.get("FT_COMPANY_PREFIX");
...
const mdItem = async (values) => {
  ...
  xml[r][tid][0].gtin = `${FTPI}:${company_prefix}.${item_ref}`; //urn:ibm:ift:product:class:2244002.00013
  xml[r][tid][0].sku = item_sku;
  // xml[r][tid][0].dataSource[0].gln = `${FTLI}:${company_prefix}.snnl`; //urn:ibm:ift:location:loc:2244002.snnl
  // xml[r][tid][0].dataRecipient[0].gln = `${FTLI}:${company_prefix}.snnl`; //urn:ibm:ift:location:loc:2244002.snnl
...
const mdFacility = async (values) => {
  ...
  xml[r].party[0].registeringParty[0] = `${FTLI}:${company_prefix}.growth`;
  xml[r].party[0].gln[0] = `${FTLI}:${company_prefix}.${loc_ref}`;
...
const epcisCommission = async (values) => {
  ...
  const xmlFilePath = path.join(cwd, "XMLMessages", "EPCIS_Commission.xml");
  const xmlFile = await fs.readFile(xmlFilePath);
  const xml = await parseString(xmlFile);

  // const bizLocation = `${FTLI}:${biz_loc}`;
  const item_epc = `${LFTPI}:${item_ref.split(":").pop()}.${item_lot}`;
  // const source = `${SFTLI}:${src_loc}.0`;
  // const dest = `${SFTLI}:${dest_loc}.0`;

  console.log(item_ref, item_lot);
  console.log(item_epc);
  console.log("urn:ibm:ift:product:lot:class:2244002.ig001.2020");

  log("date_best " + date_best);
  ...
  xml[r][b][0][el][0][oe][0][e][0].ilmd[0][fl][0][f][0][fid][0] = biz_loc;
...
const epcisAggregation = async (values) => {
  ...
  // const bizLocation = `${FTLI}:${biz_loc}`;
  const item_epc = `${LFTPI}:${item_ref.split(":").pop()}.${item_lot}`;
  // const item_epc = `${LFTPI}:${company_prefix}.${item_ref}.${item_lot}`;
  const pallet_serial = `${FTLPN}:${company_prefix}.${pallet}`;
  log(pallet_serial);
